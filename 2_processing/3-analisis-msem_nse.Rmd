...
nse_data0 <- read.csv("../1_input/data/original/dterr-w01-w04/ismt_rank.csv") #NSE calculado con PCA por QRamond
#load("../3_output/modelos/sem_models.RData")

elsoc_sub0 <- elsoc_ams %>% 
...
################################################
elsoc_ams_complete <- elsoc_ams %>% 
  dplyr::select(idencuesta, cod_zona) %>% 
  drop_na()
haven::write_dta(elsoc_ams_complete, "../1_input/data/procesada/elsoc_ams_complete.dta")
################################################

#view(dfSummary(elsoc_sub0, plain.ascii  = FALSE, style = "grid", graph.magnif = 0.75, valid.col = FALSE))

#nse_data <- nse_data0 %>% dplyr::select(idencuesta, ismt_rank)

#elsoc_sub <- dplyr::left_join(x = elsoc_sub0, y = nse_data, by = "idencuesta")

# Estandarización de variables
elsoc_sub <- elsoc_sub0 %>% mutate_at(vars(spbi, soci, segui, repbi, desgr, essui, edadi, educi, time, ismt_rank), scale)
```
...
elsoc_sub %>% 
  dplyr::select(spbi, soci, ismt_rank) %>% 
  psych::pairs.panels(smooth = TRUE,      # If TRUE, draws loess smooths
             scale = FALSE,      # If TRUE, scales the correlation text font
             density = TRUE,     # If TRUE, adds density plots and histograms
             ellipses = F,       # If TRUE, draws ellipses
...
level: 2
      ...
      spbi ~ b1*repbi + b2*sacci + b3*segui  + b4*desgr + c1*ismt_rank
      repbi ~ a1*ismt_rank
      sacci ~ a2*ismt_rank
      segui ~ a3*ismt_rank
      desgr ~ a4*ismt_rank
...
fit_msem_spbi <- sem(msem_spbi, data = elsoc_sub, cluster = "cod_zona", verbose = TRUE, optim.method = "em")
...
level: 2
      # Modelo estructural nivel 2
      soci ~ b1*repbi + b2*sacci + b3*segui  + b4*desgr + c1*ismt_rank
      repbi ~ a1*ismt_rank
      sacci ~ a2*ismt_rank
      segui ~ a3*ismt_rank
      desgr ~ a4*ismt_rank
...
coef_msem <- coef_msem %>% 
  mutate(term = recode(term, 
                       "spbi ~ repbi" = "Reputación", "soci ~ repbi" = "Reputación",
                       "spbi ~ segui" = "Seguridad", "soci ~ segui" = "Seguridad",
                       "spbi ~ sacci" = "Satisfacción", "soci ~ sacci" = "Satisfacción",
                       "spbi ~ desgr" = "Conflicto", "soci ~ desgr" = "Conflicto",
                       
                       "spbi ~ ismt_rank" = "NSE barrio",  "soci ~ ismt_rank" = "NSE barrio",
                       "spbi ~ dens_pob" = "Densidad", "soci ~ dens_pob" = "Densidad",
...
coef_plot <- coef_msem %>% 
  ggplot(aes(x = factor(term, levels = c('NSE barrio', "Densidad", 
...
ggsave(coef_plot,filename = "../3_output/graficos/coef-nse-cob-iter.png", width = 8, height = 8, dpi = 300)
```
