...
elsoc_sub %>% 
  dplyr::select(apbi, soci, ismt_rank, theil_mixt, jane_indx) %>% 
  psych::pairs.panels(smooth = TRUE,      # If TRUE, draws loess smooths
             scale = FALSE,      # If TRUE, scales the correlation text font
             density = TRUE,     # If TRUE, adds density plots and histograms
             ellipses = F,       # If TRUE, draws ellipses
...
level: 2
      ...
      apbi ~ b1*repbi + b2*sacci + b3*segui  + b4*desgr + c1*ismt_rank
      repbi ~ a1*ismt_rank
      sacci ~ a2*ismt_rank
      segui ~ a3*ismt_rank
      desgr ~ a4*ismt_rank
      ...
      ind_ismt_segui_apbi := a3*b3
      ind_ismt_desgr_apbi := a4*b4

      # Efecto total 
      tot_ismt_repbi_apbi := c1 + (a1*b1)
      tot_ismt_sacci_apbi := c1 + (a2*b2)
      tot_ismt_segui_apbi := c1 + (a3*b3)
      tot_ismt_desgr_apbi := c1 + (a4*b4)
'
...
coef_msem <- coef_msem %>% 
  mutate(term = recode(term, 
                       ...
                       "ind_ismt_segui_apbi := a3*b3" = "Nse barrio~\nSeguridad",
                       "ind_ismt_desgr_apbi := a4*b4" = "Nse barrio~\nConflicto",
...
coef_plot <- coef_msem %>% 
  ggplot(aes(x = factor(term, levels = c('Nse barrio', 
...
ggsave(coef_plot,filename = "../3_output/graficos/coef_plot_ismt_cob.png", width = 8, height = 6, dpi = 300)
```
