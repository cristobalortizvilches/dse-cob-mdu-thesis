...
elsoc_gs_short <- elsoc_gs %>% 
  dplyr::select(idencuesta, comuna_w06, comuna_cod_w06, dist, zona, cod_zona, comuna=comuna_w06,cod_comuna=comuna_cod_w06, manzana, #ids
         ...
         theil_pola=hi_theil, ismt_rank=ismtpn, gse_zona = ave_gse, ent_mean=ENTMEAN, theil_mixt=ENTWGT, theil_segr=ent_segr, ave_jane) %>%  #desigualdad
  mutate(nse_zonas = factor(car::recode(gse_zona, "c('AB', 'C1')=1; c('C2','C3')=2; c('D','E')=3"),
                      labels = c("Alto", "Medio", "Bajo")),
         jane_indx = rescale(ave_jane, to = c(0,1)))
