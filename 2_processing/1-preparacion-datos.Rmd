...
jane <- readxl::read_excel("../1_input/data/original/ismt_jane.xlsx")
...
jane <- jane %>% rename("cod_zona" = "GEOCODIGO") #renombrar variable para posterior join
jane$cod_zona <- as.numeric(jane$cod_zona)
ismt <- ismt %>% rename("cod_zona" = "geocode")
ismt$cod_zona <- as.numeric(ismt$cod_zona)

ismt_jane <- dplyr::left_join(x = ismt, y = jane, by = "cod_zona")

elsoc_d <- dplyr::left_join(x = elsoc_c, y = ismt_jane, by = "cod_zona")
```
...
elsoc_gs <- elsoc_d %>% 
  ...
  dplyr::select(idencuesta, #id individual
                ...
                jh_esc_sd, hi_theil, ismtpn, prom_ismt, gse_prom, gse_dom, ave_gse = AVE_GSE, ENTMEAN, ENTWGT, ave_jane = Ave_jan_nd, #desigualdad socioespacial
...
elsoc_gs_short <- elsoc_gs %>% 
  dplyr::select(idencuesta, comuna_w06, comuna_cod_w06, dist, zona, cod_zona, comuna=comuna_w06,cod_comuna=comuna_cod_w06, manzana, #ids
         ...
         hete=jh_esc_sd, segr, hi_theil, ismtpn,  gse_prom, gse_dom, prom_ismt, ave_gse, ent_mean = ENTMEAN, ent_wgt = ENTWGT, ent_segr, ave_jane) #segregación
