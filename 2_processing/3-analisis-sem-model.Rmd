...
elsoc_sub <- elsoc_ams %>% 
  dplyr::select(idencuesta, cod_zona, 
                ...
                hete, segr, hi_theil, ismtpn, gse_prom, gse_dom, prom_ismt, ave_gse, ent_mean, ent_wgt, ent_segr, ave_jane) %>% 
  drop_na()
...
```{r msem-low, message=FALSE, warning=FALSE}
...
```{r msem-middle-low, message=FALSE, warning=FALSE}
...
coef_apbi_class <- rbind(coef_apbi_low, coef_apbi_midlow, coef_apbi_midhigh) %>% 
  data_frame(var = c(rep("Pertenencia")))

coef_soci_class <- rbind(coef_soci_low, coef_soci_midlow, coef_soci_midhigh) %>% 
  data_frame(var = c(rep("Sociabilidad")))
...
```{r save-models}
save(fit_msem_apbi, fit_sem_apbi_low, fit_sem_apbi_midlow, fit_sem_apbi_midhigh,
     fit_msem_soci, fit_sem_soci_low, fit_sem_soci_midlow, fit_sem_soci_midhigh,
