...
```{r merge-datasets}
# I. Selección previa de variables

## ZONAS ELSOC
id_territorio <- manzanas_2017_elsoc %>% select(idencuesta, cod_zona = geocodigo_ok)

## ISMT
ismt_rm <- ismt_data %>% dplyr::select(cod_zona = zona, ismt_rank = ismtpn) #selección y renombre de variables

# II. Unión de base de datos

## JOIN: ELSOC & ZONAS
id_territorio$idencuesta <- as.numeric(id_territorio$idencuesta) #cambio clase para hacer el siguiente join
elsoc_a <- dplyr::left_join(x = elsoc_wide_2016_2022.2, y = id_territorio, by = "idencuesta")

## JOIN: ELSOC-ZONAS & ISMT
elsoc_a$cod_zona <- as.numeric(elsoc_a$cod_zona) #cambio clase para hacer el siguiente join
elsoc_b <- dplyr::left_join(x = elsoc_a, y = ismt_rm, by = "cod_zona")
```
...
elsoc_ams <- elsoc_b %>% 
  dplyr::select(idencuesta, #id individual
                ...
                t02_01_w06, t02_02_w06, t02_03_w06, t02_04_w06, #pertenencia barrial 
                t03_01_w06, t03_02_w06, t03_03_w06, t03_04_w06, #sociabilidad barrial
                t06_02_w06, t06_05_w06, t06_06_w06, t06_07_w06, #satisfacción barrial
                t08_w06, t10_w06, t04_06_w04, # reputación | seguridad | conflicto barrial 
                ismt_rank, #desigualdad socioespacial
                m0_sexo_w06, m0_edad_w06, #sexo | edad
                m01_w06, d01_01_w06, m29_w06, m14_w06, # escolaridad | estatus subjetivo | 
                m34_03_w03) %>% # tiempo de residencia
...
elsoc_nse <- elsoc_b %>% 
  dplyr::select(idencuesta, #id individual
                estrato_w06, comuna_cod_w06,
                m01_w06, # escolaridad
                d01_01_w06, # estatus subjetivo
                m29_w01, m29_w02, m29_w03, m29_w04, m29_w05, m29_w06) %>% #ingreso total hogar (monto)
  filter(estrato_w06 == 1 &  #solo Gran Santiago en la medición 2022
         comuna_cod_w06 < 13605 & # excluye comunas rurales del RM
         comuna_cod_w06 > 11000 & # excluye comunas fuera de la RM
         !comuna_cod_w06 %in% c(13602, 13603))
  
elsoc_nse[elsoc_nse == -888 | elsoc_nse == -999 | elsoc_nse == 994 | elsoc_nse == 995 | elsoc_nse == 996 | elsoc_nse == -666 | elsoc_nse == -777] <- NA # varias categoría a NA

elsoc_nse  <- elsoc_nse %>% 
  mutate(ingr_impu = coalesce(m29_w06, m29_w05, m29_w04, m29_w03, m29_w02, m29_w01),
         ingr_prom = rowMeans(dplyr::select(., starts_with("m29_")), na.rm = TRUE))

view(dfSummary(elsoc_nse, plain.ascii  = FALSE, style = "grid", graph.magnif = 0.75, valid.col = FALSE))
View(elsoc_nse)
...
elsoc_ams <- elsoc_ams %>% 
  ...
  #-----Recode M: mecanismos subjetivos 
         segu = factor(car::recode(t10_w06, "1:2=1;3=2;4:5=3"),
                       labels = c("Baja",  "Media", "Alta"), levels = 1:3),
         segui = t10_w06,
         sacci = rowMeans(dplyr::select(., t06_02_w06, t06_05_w06, t06_06_w06, t06_07_w06), na.rm = TRUE),
...
elsoc_ams <- elsoc_ams %>% 
  ...
  mutate_at(vars(spbi, soci, segui, repbi, desgr, essui, edadi, educi, time, ismt_rank), scale)
```
