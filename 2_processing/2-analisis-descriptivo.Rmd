...
scatter_jane_cob <- ggarrange(scat_apbi_jane, scat_soci_jane,  ncol = 2, common.legend = TRUE, legend = "top")
...
```{r}
scat_apbi_nse <- elsoc_mean_zonas %>% 
  ggplot(aes(x = ismt_rank, y = apbi, color = theil_mixt_f)) +
  geom_point(size = 0.8) +
  geom_smooth(method = "lm", se = F, size = 0.8) +
  #geom_smooth(se = F, size = 1.5) +
  geom_smooth(aes(x = ismt_rank, y = apbi), 
              method = "lm", se = F, inherit.aes = F, colour = "black", size = 1, linetype = "twodash") +
  labs(x = "Nse barrio", 
       y = "Pertenencia",
       color = "Mixtura") +
...
scatter_nse_cob <- ggarrange(scat_apbi_nse, scat_soci_nse, ncol = 2, common.legend = TRUE, legend="top")
