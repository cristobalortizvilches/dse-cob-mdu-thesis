...
prom_zonas <- elsoc_ams %>% 
  group_by(cod_zona) %>% 
  dplyr::select(apbi, soci, segui, repbi, sacci, educi, essui, hete, segr, ismtpn, gse_prom, gse_dom, ent_mean, ent_wgt, ave_jane) %>% 
...
elsoc_ams %>% 
  ggplot(aes(x = ent_wgt, y = apbi)) +
  geom_boxplot() +
  labs(x = "", y = "")

elsoc_ams %>% 
  ggplot(aes(x = ent_wgt, y = soci)) +
  geom_boxplot() +
  labs(x = "", y = "")
```
...
scat_apbi_jane <- prom_zonas %>% 
  ...
  geom_point() +
  geom_smooth() +
  labs(x = "Nivel de vitalidad barrial", 
       y = "Promedio de apego barrial")
...
mapview(elsoc_ams_all_geo, zcol = "soci")
```
